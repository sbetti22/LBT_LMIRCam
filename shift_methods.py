...
written by Kim Ward-Duong Feb 2018
...
#        im2_gray = im2_gray[0:kwargs['boxsize'],0:kwargs['boxsize']]
    ...
    im1_gray[np.isnan(im1_gray) | np.isinf(im1_gray)] = np.nanmedian(im1_gray)
    im2_gray[np.isnan(im2_gray) | np.isinf(im2_gray)] = np.nanmedian(im2_gray)
